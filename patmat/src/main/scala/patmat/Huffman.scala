...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: rest => (makeCodeTree(left, right) :: rest).sortWith((x, y) => weight(x) < weight(y))
    case _ => trees
  }
  ...
  /**
    ...
    */
  def until(checker: List[CodeTree] => Boolean, combiner: List[CodeTree] => List[CodeTree])(trees: List[CodeTree])
  : List[CodeTree] = {
    if (checker(trees)) trees
    else until(checker, combiner)(combiner(trees))
  }
